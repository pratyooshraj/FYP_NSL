...
class GestureRecognitionApp:
    def __init__(self, root):
        ...
        self.capture_button = tk.Button(self.control_frame, text="Capture Gesture", command=self.capture_gesture,
                                        state="disabled")
        ...
        # Sentence display area and buttons
        ...
        self.sentence_control_frame = tk.Frame(self.root)
        self.sentence_control_frame.pack(pady=10)

        self.save_button = tk.Button(self.sentence_control_frame, text="Save Sentence", command=self.save_sentence,
                                     state="disabled")
        self.save_button.grid(row=0, column=0, padx=10)

        self.clear_button = tk.Button(self.sentence_control_frame, text="Clear Sentence", command=self.clear_sentence,
                                      state="disabled")
    ...
    def update_sentence(self, gesture_label):
        ...
        # Enable save and clear buttons when the sentence updates
        self.save_button.config(state="normal")
        self.clear_button.config(state="normal")

    def save_sentence(self):
        """
        Save the current sentence to a text file and convert it to voice output.
        """
        if not self.sentence.strip():
            messagebox.showinfo("Info", "No sentence to save.")
            return
        ...
        if file_path:
            ...
            self.log_message(f"Sentence saved to {file_path}")

            # Convert to speech
            self.tts_engine.say(self.sentence)
            self.tts_engine.runAndWait()
            messagebox.showinfo("Info", "Sentence saved and spoken.")

    def clear_sentence(self):
        """
        Clear the current sentence and reset the gesture queue.
        """
        ...
        self.log_message("Sentence cleared.")
